...
import type { AuthorOption, EditablePost } from "@/types/blog";
...
export function EditPostForm({ post }: { post: EditablePost }) {
    ...
    useEffect(() => {
        ...
        supabase
            ...
            .then(({ data }) => {
                if (data) {
                    const fetchedCategories = (data as Array<{
                        id: string | number;
                        name: string;
                    }>).map((item) => ({
                        id: String(item.id),
    ...
    const handleSave = () => {
        ...
        startTransition(async () => {
            try {
                ...
                console.error(error);
                alert("Erro ao atualizar o post: " + getErrorMessage(error));
            }
        });
    };
        ...
        <div className="max-w-5xl mx-auto pt-28 lg:pt-0">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    <Link
                        href="/admin/posts"
                        className="flex h-9 w-9 items-center justify-center rounded-xl border border-forest-200 text-[#3B5A3C] hover:text-[#0D1812] hover:bg-[#EEEDE5] transition-all no-underline"
                    >
                        <ArrowLeft className="h-4 w-4" />
                    </Link>
                    <div>
                        <h1 className="font-display text-2xl font-bold text-[#0D1812]">
                            Editar Artigo
                        </h1>
                        <div className="flex items-center gap-2 mt-1">
                            <span
                                className={`inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-semibold uppercase tracking-wider ${
                            ...
                            <Link
                                href={`/blog/${slug}`}
                                className="flex items-center gap-1 text-xs text-[#E8D49A] hover:text-[#877249] no-underline"
                                target="_blank"
                            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    <div>
                        <label
                            htmlFor="title"
                            className="block text-xs font-semibold uppercase tracking-[0.1em] text-[#3B5A3C] mb-2"
                        >
                            Título do Artigo
                        </label>
                        <input
                    ...
                    <div>
                        ...
                        <div className="flex items-center rounded-xl border border-forest-200 bg-white overflow-hidden focus-within:ring-2 focus-within:ring-champagne-500/50 focus-within:border-[#E8D49A] transition-all">
                            <span className="px-3 text-xs text-[#3B5A3C]/70 bg-[#EEEDE5]/60 py-3 border-r border-forest-200 shrink-0">
                                /blog/
                            </span>
                            <input
                                id="slug"
                                ...
                                disabled
                                className="flex-1 px-3 py-3 text-sm text-[#0D1812] focus:outline-none opacity-50 cursor-not-allowed"
                            />
                        </div>
                    </div>
                    <div>
                ...
                <div className="space-y-6">
                    <div className="rounded-2xl bg-white border border-forest-100 p-5">
                        <label
                            htmlFor="coverImage"
                            className="block text-xs font-semibold uppercase tracking-[0.1em] text-[#3B5A3C] mb-3"
                        >
                            Imagem de Capa
                        </label>
                        <input
                    ...
                    <div className="rounded-2xl bg-white border border-forest-100 p-5">
                        ...
                        {selectedAuthorPhoto && selectedAuthor && (
                            <div className="mt-4 flex items-center gap-3 p-3 rounded-xl border border-forest-100 bg-[#EEEDE5]/20">
                                ...
                                <span className="text-sm font-medium text-[#0D1812]">
                    ...
                    <div className="rounded-2xl bg-[#1B2D1E] p-5">
                        ...
                        <div className="space-y-1">
                            <p className="text-blue-400 text-sm font-medium truncate">
                                {title} | Erlo, Haas & Steffens
                            </p>
                            <p className="text-green-400 text-xs truncate">
                                erlohaas.com.br/blog/{slug}
                            </p>
